...
const defaultFormData = {
    choose_flow: "i_have_destination",
    country: [],
    first_time_visit: "",
    season_name: "",
    travel_month: "",
...
export function LandingPlanTripModal({ open, onOpenChange }: Props) {
    // Define route
    ...
    useEffect(() => {
        // Filter current path
        let currentCountry = currentPath.replace("/country/", "").replace("-", " ");

        // Convert every word uppercase
        currentCountry = currentCountry.replace(/(?:^|\s)\S/g, l => l.toUpperCase());

        // Update state
        setPlanYourTripForm({ ...planYourTripForm, country: [currentCountry] });
    }, [currentPath]);
